		...
		if string.sub(message, 1, 6) == "!mute " then
			...
			if target then
				if playerAccountType > target:getAccountType() then
					if not target:getCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_ADVERTISING) then
						target:addCondition(muted)
						target:setStorageValue(storage, os.time() + 180)
						sendChannelMessage(CHANNEL_ADVERTISING, TALKTYPE_CHANNEL_R1, target:getName() .. " has been muted by " .. player:getName() .. " for using Help Channel inappropriately.")
					else
			...
			if target then
				if playerAccountType > target:getAccountType() then
					if target:getStorageValue(storage) > os.time() then
                        target:removeCondition(CONDITION_CHANNELMUTEDTICKS, CONDITIONID_DEFAULT, CHANNEL_ADVERTISING)
                        sendChannelMessage(CHANNEL_ADVERTISING, TALKTYPE_CHANNEL_R1, target:getName() .. " has been unmuted.")
